...
`replay_versions (replay_version smallint pk, label text, semantics jsonb not null, code_hash text not null, activated_at, retired_at)`; authenticated SELECT, service_role write. Version 1 = `legacy_m15_optimistic`, whose `code_hash` is the frozen pre-migration hash verified at Checkpoint B (§13). Version 2 = `m15_fail_closed_actual_risk`, whose `semantics` records: TIF interval rule, detection-bar rule (strictly after `detected_at`, unchanged), actual-risk normalisation rule, favorable-gap-only limit semantics, stop-gap rule (§8), M15 horizontal-ambiguity policy (§7), **chronology-vs-policy rule (§7b)**, **same-fill-bar target causality and target-analytics rule (§7a)**, **fill-bar MFE/MAE exclusion rule (§10)**, **candle-reuse source (§12)**, supported execution policy and its post-exit analytics boundary (§12a), gross/net cost convention, and `code_hash` over the v2 replay code plus `ORDER_TIF_MINUTES` / `SIGNAL_MAX_AGE_HOURS`. Once any Replay-V2 row exists, changing any of these requires Replay V3, enforced by a blocking hash test.
...
1. Baseline Checkpoint A (pre-migration). 2. Additive migration + `plan_id` backfill + registry seed + **all** consumer filters (§11) deployed together with both research flags off. 3. Checkpoint B verification (§13). 4. `replaySetupV2` with the full fixture matrix. 5. Candle-reuse research phase wired into the resolver, then `replay_v2_shadow_enabled = true`. 6. `replay_v2_backfill_enabled = true` to widen the research slice to historical plans. 7. Admin paired panel (McNemar on paired labels, QA counters, coverage, gross-only R). 8. Promotion is a separate prompt: `regime_stats` replay dimension, version-specific milestone state, gate report, then one row update to `active_replay_version`.
