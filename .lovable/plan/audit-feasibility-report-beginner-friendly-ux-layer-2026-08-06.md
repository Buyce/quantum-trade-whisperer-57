...
The Audit & Feasibility Report is APPROVED! Please execute the implementation scope as planned:

1. Mount TooltipProvider in `src/routes/__root.tsx`.

2. Create `src/components/GuideMode.tsx` (Context + Header Toggle button next to Sign Out + <InfoLabel> component).

3. Create `src/components/OnboardingBanner.tsx` (using localStorage key `ptrades.onboarding.dismissed` in useEffect).

4. Update `SignalCard.tsx`:

   - Add the multi-line "Copy Order Details" button to the footer flex row.

   - Conditionally render execution badges ("BUY LIMIT (LONG)" / "SELL LIMIT (SHORT)") when Guide Mode is ON, falling back to "LONG" / "SHORT" when OFF.

   - Wrap metric headers in <InfoLabel> for contextual tooltips in Guide Mode.

5. Update `src/routes/_authenticated/feed.tsx` to include the Onboarding Banner and an enriched "Capital Preservation Mode" empty state in Guide Mode.

6. Update `src/routes/_authenticated/performance.tsx` with metric tooltips in Guide Mode.