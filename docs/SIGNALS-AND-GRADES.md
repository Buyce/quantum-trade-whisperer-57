...
Planned entry (a limit at the M15 structural break, shifted toward the breakout
close only during the London/New York overlap, where a deep Point-C retest is
historically rare); maximum acceptable entry (slippage ceiling — `0.15R` normally,
...
`maxR`, the maximum reachable R before the nearest H4 barrier; a `capped` flag
...
Every field is computed from broker candles. Nothing is user-supplied and nothing
is estimated.

## Failure behaviour

If a structure cannot reach a third target before the H4 barrier, `tp3` and
