...
| `list_broker_trades`      | read   | the user's BROKER-CONFIRMED trades from broker evidence — the authority on what actually happened. Windowed on the broker exit time, filterable by state and instrument, sortable by exit time or by R (best/worst trade)                              |
