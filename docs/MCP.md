...
`get_my_settings` also returns `auto_cohort_policies`: the owner's per-pair and
per-direction automatic-order rules. `update_my_settings` writes them through the
same field, requires `confirm_risk_change: true`, and returns a warning whenever a
restriction is loosened. Only cohorts the owner changed are stored; anything
absent is allowed at their normal risk. The rule is reduce-only — see
[RISK-GUARDIAN.md](RISK-GUARDIAN.md).
