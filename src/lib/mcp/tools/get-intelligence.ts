...
export interface GetIntelligenceArgs {
...
export async function runGetIntelligence(supabase: unknown, input: GetIntelligenceArgs) {
  ...
  if (input.signal_id) {
    ...
    if (!signal) {
      return {
        content: [{ type: "text" as const, text: `No signal ${input.signal_id} found.` }],
        isError: true,
      };
    }
...
export default defineTool({
  ...
  handler: async (input, ctx) => {
    ...
    return runGetIntelligence(supabaseForUser(ctx), input);
  },
});
