...
export interface PositionSizeArgs {
...
export async function runCalculatePositionSize(
  supabase: unknown,
  userId: string,
  args: PositionSizeArgs,
) {
  const db = supabase as ReturnType<typeof supabaseForUser>;
  let symbol = args.instrument;
  let entry = args.entry_price;
  let stop = args.stop_loss;
  ...
  if (args.signal_id) {
    ...
    if (!signal) {
      return {
        content: [{ type: "text" as const, text: `No signal ${args.signal_id} found.` }],
        isError: true,
      };
    }
  ...
  const sizing = await resolveSizingForUser(db, userId, {
    instrument: symbol,
    entryPrice: entry,
    stopLoss: stop,
    finalTargetR,
    signalId: args.signal_id ?? null,
  });
...
export default defineTool({
  ...
  handler: async ({ signal_id, instrument, entry_price, stop_loss }, ctx) => {
    ...
    return runCalculatePositionSize(supabaseForUser(ctx), ctx.getUserId() as string, {
      signal_id,
      instrument,
      entry_price,
      stop_loss,
