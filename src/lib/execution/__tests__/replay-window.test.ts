...
describe("barsRequiredForRow", () => {
  it("counts bars from the replay cursor when one exists", () => {
    const row = {
  ...
  it("falls back to detection time when there is no cursor", () => {
    const row = {
  ...
  it("returns null for an unreadable start point rather than a number", () => {
    expect(barsRequiredForRow({ detected_at: "not a date", replay_cursor: null }, now)).toBeNull();
  });
});

describe("classifyReplayWindow", () => {
  it("labels a row that needs more history than the cap allows", () => {
    const row = {
  ...
  it("leaves a row inside the window unlabelled", () => {
    const row = {
  ...
  it("does not label an exactly-at-cap row", () => {
    const row = {
  ...
  it("never labels a row whose start point could not be read", () => {
