...
describe("percentiles", () => {
  ...
  it("uses nearest rank for p90, not an average", () => {
...
describe("scoreWindow", () => {
  it("marks a thin window as not measured and says why", () => {
  ...
  it("never treats unavailable slippage or R as zero", () => {
    const score = scoreWindow(
  ...
  it("counts margin refusals from the broker's own reason text", () => {
    const score = scoreWindow(
      {
  ...
  it("reports no reject rate at all when there were no deliveries", () => {
    const score = scoreWindow(
...
describe("evaluateCooldown", () => {
  ...
  it("does not breach when the recent window is unmeasured", () => {
  ...
  it("does not breach when there is no norm to breach", () => {
  ...
  it("breaches on slippage only against the dimension's own norm", () => {
  ...
  it("does not breach when slippage is merely a little worse", () => {
    expect(evaluateCooldown(measuredRecent(0.00015), measuredNorm(0.0001), NOW).breached).toBe(
      false,
    );
  });

  it("breaches on reject rate once enough recent deliveries exist", () => {
  ...
  it("ignores a bad reject rate on too few recent deliveries", () => {
    const verdict = evaluateCooldown(
...
describe("splitWindows", () => {
  it("separates recent from norm and drops rows older than the norm window", () => {
    const rows = [
      { atMs: NOW - 1 * DAY },
