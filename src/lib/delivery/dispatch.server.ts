...
export async function processNextDelivery(
  ...
  // confirmation lives on the row itself, so for a Live Confirm destination it is
  // read here and merged: without it a confirmed order would look unconfirmed and
  // be refused. Any other destination is untouched.
  let delivery: DeliveryRow = claimed;
  if (claimed.account_mode === "live_confirm") {
    const { data: confirmationRow } = await db
      ...
      .eq("id", claimed.id)
      .maybeSingle();
    delivery = { ...claimed, ...((confirmationRow ?? {}) as Partial<DeliveryRow>) };
  }
