...
describe("spreadNorm", () => {
  it("[UNIT] refuses to claim a norm from too few trading days", () => {
    const norm = spreadNorm([day(1, 0.00002), day(2, 0.00002)]);
    expect(norm.measured).toBe(false);
  });

  it("[UNIT] refuses to claim a norm from too few samples", () => {
    const norm = spreadNorm([1, 2, 3, 4, 5].map((d) => day(d, 0.00002, 2)));
    expect(norm.measured).toBe(false);
  });

  it("[UNIT] ignores rows with no usable p90", () => {
...
describe("effectiveSpreadCeiling", () => {
  ...
  it("[UNIT] leaves a disabled gate disabled", () => {
    expect(effectiveSpreadCeiling(0, 0.0001, measured).pips).toBe(0);
  });

  it("[INVARIANT] never widens the owner's ceiling", () => {
  ...
  it("[UNIT] tightens when the instrument's own normal spread is tighter", () => {
  ...
  it("[INVARIANT] never tightens below the bounded floor", () => {
  ...
  it("[UNIT] does nothing without a measured norm or without a pip size", () => {
