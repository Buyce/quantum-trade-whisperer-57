...
describe("readCeilingSettings", () => {
  it("treats absent, zero and negative values as disabled", () => {
...
describe("pipSizeFromSpec", () => {
  it("reads a 5-digit FX pip as ten broker points", () => {
    expect(pipSizeFromSpec({ point: 0.00001, digits: 5 })).toBeCloseTo(0.0001, 10);
  });

  it("reads a 2-digit instrument pip as one point", () => {
    expect(pipSizeFromSpec({ point: 0.01, digits: 2 })).toBeCloseTo(0.01, 10);
  });

  it("returns null when the broker published neither point nor digits", () => {
...
describe("slippageWithinUserCeiling", () => {
  ...
  it("passes when price is close to the published entry", () => {
    expect(slippageWithinUserCeiling(3, pip, 1.2, 1.2002).ok).toBe(true);
  });

  it("refuses when price has moved past the owner's tolerance", () => {
    const v = slippageWithinUserCeiling(2, pip, 1.2, 1.2005);
    expect(v.ok).toBe(false);
  });

  it("refuses when the pip size is unknown", () => {
...
describe("exposurePercentWithinCeiling", () => {
  ...
  it("is disabled when no ceiling is configured", () => {
    expect(exposurePercentWithinCeiling(0, true, none, null).ok).toBe(true);
  });

  it("refuses when the incoming order's risk percent is unknown", () => {
    const v = exposurePercentWithinCeiling(2, true, none, null);
    expect(v.ok).toBe(false);
  });

  it("allows a total inside the ceiling", () => {
  ...
  it("blocks an over-ceiling total when enforcement is on", () => {
    const v = exposurePercentWithinCeiling(1, true, { knownPercent: 0.8, unknownOrders: 0 }, 0.5);
    expect(v.ok).toBe(false);
  });

  it("reports but does not block when enforcement is off", () => {
  ...
  it("says out loud that unrecorded earlier orders make the total a floor", () => {
