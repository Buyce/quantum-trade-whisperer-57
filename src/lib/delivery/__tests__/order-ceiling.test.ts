...
describe("occupiedOrderCounts", () => {
  it("[INVARIANT] a dry-run delivery never spends the ceiling", async () => {
    const c = client([
      ...
      { user_id: "u1", state: "pending", enqueued_at: "2026-08-25T18:40:00Z", dry_run: false },
    ]);
  ...
  it("[INVARIANT] real occupying orders are still counted", async () => {
    const c = client([
      { user_id: "u1", state: "sent", enqueued_at: "2026-08-25T18:00:00Z", dry_run: false },
      { user_id: "u1", state: "acknowledged", enqueued_at: "2026-08-25T18:10:00Z", dry_run: false },
      { user_id: "u2", state: "pending", enqueued_at: "2026-08-25T18:20:00Z", dry_run: false },
    ]);
