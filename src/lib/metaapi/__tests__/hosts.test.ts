...
describe("metaapi host resolution", () => {
  ...
  it("[UNIT] resolves global services without a region", () => {
    expect(resolveHost("provisioning")).toBe("https://mt-provisioning-api-v1.agiliumtrade.ai");
    expect(resolveHost("metastats")).toBe("https://metastats-api-v1.agiliumtrade.ai");
    expect(resolveHost("risk-management")).toBe("https://risk-management-api-v1.agiliumtrade.ai");
  });
  ...
  it("[INVARIANT] only hosts this resolver can produce are trusted", () => {
    ...
    expect(isTrustedMetaApiHost("https://mt-provisioning-api-v1.agiliumtrade.ai")).toBe(true);
