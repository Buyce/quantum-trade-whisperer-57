  ...
  return queryOptions({
    ...
    queryFn: async (): Promise<BrokerOrderView[]> => {
      ...
      const { data: recovered, error: recoveredError } = await supabase
        ...
        .select(
          "id, client_id, broker_symbol, first_observed_at, state, broker_account_type, direction, volume, entry_price, exit_price, entry_at, exit_at, gross_profit, commission, swap, profit_currency, r_vs_plan, r_vs_actual_risk, r_availability, stop_provenance, published_entry, slippage_price, slippage_availability, slippage_basis, signal_instrument, signal_grade, signal_grade_source",
        )
        .is("delivery_id", null)
...
export const SETTINGS_SELECT =
  "user_id, instruments, timeframes, sessions, min_grade, alert_min_grade, daily_setup_cap, notify_push, notify_email, order_strategy, webhook_enabled, webhook_url, webhook_format, execution_enabled, execution_dry_run, exposure_limit_enabled, webhook_validated_at, webhook_validation_reason, account_equity, account_currency, risk_per_trade_percent, max_position_size, leverage, max_stop_loss_percent, equity_as_of, risk_ack_high, auto_intel_gate_enabled, auto_intel_min_win_pct, auto_intel_min_sample, auto_intel_min_expected_r, auto_execute_c_grade, maximum_active_signal_orders, maximum_concurrent_signal_orders, maximum_daily_signal_orders, auto_market_entry_enabled, auto_exit_policy, auto_exit_shares, auto_exit_trail_runner, allow_unmeasured_intel, auto_order_window_minutes, maximum_daily_orders_per_symbol, adaptive_order_ceilings_enabled, adaptive_order_ceiling_max, adaptive_order_ceiling_floor, max_entry_spread_pips, max_entry_slippage_pips, max_total_exposure_percent, drawdown_brakes_enabled, daily_loss_limit_percent, weekly_loss_limit_percent, consecutive_loss_limit, consecutive_loss_pause_hours, max_drawdown_percent, max_same_bet_orders, same_bet_cooldown_minutes";
