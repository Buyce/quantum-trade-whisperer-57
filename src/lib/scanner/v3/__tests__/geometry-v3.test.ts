...
describe("V3 target-preserving slippage allowance", () => {
  ...
  it("[UNIT] is zero at exactly k = m", () => {
    expect(d(2)).toBe(0);
  });

  it("[UNIT] binds on the ratio term when it is tighter than the cap", () => {
    // k = 2.3, m = 2 => 10*0.3/3 = 1.0 < the 0.15R = 1.5 cap.
    expect(d(2.3)).toBeCloseTo(1.0, 6);
  });

  it("[UNIT] binds on the 0.15R cap once the ratio term is looser", () => {
    // k = 2.5, m = 2 => 10*0.5/3 = 1.667, so the cap wins.
    expect(d(2.5)).toBeCloseTo(1.5, 6);
  });

  it("[UNIT] never exceeds the 0.15R cap for a wide extension", () => {
    expect(d(8)).toBeLessThanOrEqual(0.15 * r + 1e-9);
  });

  it("[UNIT] uses the thin ratio floor below maxR 1.5", () => {
    // k = 1.4 < 1.5 => m = 1 => 10*0.4/2 = 2.0, capped to 1.5.
    expect(d(1.4)).toBeCloseTo(1.5, 6);
  });

  it("[UNIT] keeps the realised ratio at or above the floor at the allowance limit", () => {
  ...
  it("[UNIT] fails closed on degenerate inputs", () => {
...
describe("V3 max acceptable entry", () => {
  it("[UNIT] returns the entry itself, flagged limit-only, at maxR 1.6", () => {
  ...
  it("[UNIT] widens away from the stop for both directions", () => {
    const long = maxAcceptableEntryV3({
      entryPrice: 1.1,
      stopLoss: 1.098,
      maxR: 3,
...
describe("V3 manifest", () => {
  it("[UNIT] locks the stop window to the retracement leg and disables the offset", () => {
    expect(MODEL_V3_PARAMS.stop.window).toContain("bIndex + 1");
    expect(MODEL_V3_PARAMS.entry.dynamicOffset).toContain("disabled");
  });

  it("[UNIT] is registered under a stable code hash", () => {
    expect(MODEL_V3_CODE_HASH).toBe("3c327b029da38563");
  });

  it("[UNIT] never contributes to live priors", () => {
