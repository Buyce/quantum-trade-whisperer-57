 ...
 *
 * `structureKey` is checked separately below, also for exact equality: no field
 * of the V1 path is allowed to diverge from the frozen baseline.
 */
...
describe("pre-Prompt-7 characterization — frozen baseline vs current V1", () => {
  ...
  it.each(CASES)(
    "[V1_CHARACTERIZATION] %s — structure identity is byte-identical to the frozen key",
    (_id, scenario) => {
      const input = {
        instrument: scenario.instrument,
        candles: scenario.candles,
      ...
      // No tolerance, no re-rendering, no normalisation: duplicate detection
      // compares strings against rows already in the database, so the only
      // acceptable relationship to the frozen key is equality (Phase A2A R1-FIX).
      expect(current.structureKey).toBe(frozen.structureKey);
    },
  );
