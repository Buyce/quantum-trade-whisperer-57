...
describe("password rules", () => {
  it("[UNIT] refuses a short password before comparing the confirmation", () => {
  ...
  it("[UNIT] refuses a password beyond the bcrypt limit", () => {
    const result = validateNewPassword("a".repeat(PASSWORD_MAX_LENGTH + 1), "a");
    expect(result.ok).toBe(false);
  });

  it("[UNIT] refuses a mismatched confirmation", () => {
  ...
  it("[UNIT] accepts a valid matching pair", () => {
    expect(validateNewPassword("correct-horse", "correct-horse")).toEqual({ ok: true });
  });

  it("[UNIT] treats a Google-only account as passwordless", () => {
  ...
  it("[UNIT] only calls a recovery link expired once the session check finished", () => {
