...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Instruments          | Lifecycle-driven, because lifecycle enforcement is ON. Publishing today: `XAUUSD`, `GBPAUD`, `EURUSD` (`execution_approved`). Measured but not published: `AUDUSD`, `USDJPY` (`shadow`), `GBPUSD`, `USDCAD`, `USDCHF`, `NAS100` (`data_validation`). Off: `XAGUSD`, `USOIL`, `UKOIL`. Anything at `data_validation` or beyond is scanned | `src/lib/instruments/registry.ts`, `scanUniverse` in `src/lib/scanner/pipeline.server.ts`, `instrument_lifecycle` |
...
| Statistics           | Whole-UTC-day cluster bootstrap (2000 replicates, fixed seed), Benjamini–Hochberg, Wilson intervals for baseline rates, three named evidence tiers (`descriptive` 30/10, `chronological_period` 30/5, `training` 200/10), no forward holdout                                                                                             | `src/lib/stats/*`, `src/lib/baseline/capture.server.ts`                                                           |
| Automatic pauses     | Owner drawdown brakes measured from closed broker trades and broker equity (fail closed when unmeasurable); per account/instrument/session execution-quality cooldowns for 24h (refuse nothing when unreadable); evidence-ranked daily cap that reorders but never resizes                                                               | `src/lib/risk/brakes.ts`, `src/lib/execution/quality.ts`, `src/lib/delivery/eligibility.ts`                       |
...
| AI assistants        | 18 MCP tools over an OAuth-protected `/mcp` endpoint                                                                                                                                                                                                                                                                                     | `.lovable/mcp/manifest.json`, `src/lib/mcp/tools/*`                                                               |
