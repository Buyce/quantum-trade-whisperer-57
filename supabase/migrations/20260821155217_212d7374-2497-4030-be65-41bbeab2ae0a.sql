...
-- Admin-only reader for the research filter-lift table.
CREATE OR REPLACE FUNCTION public.get_admin_filter_lift()
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'generated_at', now(),
    'enrolment_enabled', (SELECT candidate_enrolment_enabled FROM shadow_engine_state LIMIT 1),
    'rows', coalesce((SELECT jsonb_agg(to_jsonb(f) ORDER BY f.gate, f.arm)
