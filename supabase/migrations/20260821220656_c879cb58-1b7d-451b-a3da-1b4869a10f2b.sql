-- Prompt 8/9 closure F — resolved-state immutability over the COMPLETE resolved
-- payload and provenance set. Semantically identical retries are accepted
-- (no-op); any conflicting mutation of a resolved trade is rejected.
-- Numeric comparisons round to 4dp, mirroring R_DECIMALS in
-- src/lib/journal/r-math.ts, so float noise cannot fake a conflict.
...
BEGIN
  ...
  -- 3. Resolved-state protection over the complete resolved payload set:
  --    outcome, execution prices, actual initial stop, canonical Rs,
  --    r_availability, stop_provenance, r_math_version, price provenance,
  --    execution timestamps, verification level, costs and net-R provenance.
  IF OLD.outcome IS DISTINCT FROM 'open'::trade_outcome THEN
    same :=
      NEW.outcome IS NOT DISTINCT FROM OLD.outcome
      AND NEW.trade_state IS NOT DISTINCT FROM OLD.trade_state
      ...
      AND round(coalesce(NEW.net_r, -999999)::numeric, 4)
          = round(coalesce(OLD.net_r, -999999)::numeric, 4)
      ...
      AND NEW.r_math_version IS NOT DISTINCT FROM OLD.r_math_version
      ...
      AND NEW.price_source_client IS NOT DISTINCT FROM OLD.price_source_client
      AND NEW.price_recorded_at IS NOT DISTINCT FROM OLD.price_recorded_at
      AND NEW.actual_entry_at IS NOT DISTINCT FROM OLD.actual_entry_at
      AND NEW.actual_exit_at IS NOT DISTINCT FROM OLD.actual_exit_at
      AND round(coalesce(NEW.commission, -999999)::numeric, 4)
          = round(coalesce(OLD.commission, -999999)::numeric, 4)
      AND round(coalesce(NEW.swap, -999999)::numeric, 4)
          = round(coalesce(OLD.swap, -999999)::numeric, 4)
      AND NEW.cost_currency IS NOT DISTINCT FROM OLD.cost_currency
      AND NEW.cost_unit IS NOT DISTINCT FROM OLD.cost_unit
      AND NEW.broker_ticket IS NOT DISTINCT FROM OLD.broker_ticket
      AND NEW.partial_exits IS NOT DISTINCT FROM OLD.partial_exits
      AND NEW.user_decision IS NOT DISTINCT FROM OLD.user_decision;

    IF NOT same THEN
      RAISE EXCEPTION 'trade_already_resolved'
        USING HINT = 'A resolved trade only accepts semantically identical retries. Use a correction workflow to change it.';
    END IF;
  END IF;
