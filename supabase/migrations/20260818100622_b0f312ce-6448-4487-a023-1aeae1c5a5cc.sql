-- The live scanner must classify a brand-new setup into the same volatility
-- tercile the statistics were built from, so the boundaries have to be published
-- alongside them. Tier 0 rows carry the per-instrument boundaries.
ALTER TABLE public.regime_stats
  ADD COLUMN IF NOT EXISTS vol_t1 numeric,
  ADD COLUMN IF NOT EXISTS vol_t2 numeric;
