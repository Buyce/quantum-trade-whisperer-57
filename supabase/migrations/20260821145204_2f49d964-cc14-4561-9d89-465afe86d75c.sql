-- Stage 4 (dark): cohort discriminator + enforced production isolation boundary.
ALTER TABLE public.shadow_executions
  ADD COLUMN IF NOT EXISTS cohort text NOT NULL DEFAULT 'production';

UPDATE public.shadow_executions SET cohort = 'production' WHERE cohort IS NULL;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'shadow_executions_cohort_check') THEN
    ALTER TABLE public.shadow_executions
      ADD CONSTRAINT shadow_executions_cohort_check
      CHECK (cohort IN ('production', 'research_candidate'));
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS shadow_executions_cohort_open
  ON public.shadow_executions (cohort, replay_version, detected_at)
  WHERE status IN ('pending', 'open');

-- The isolation boundary is the VIEW, not the index: every production aggregate
-- reads this and nothing else, so a forgotten WHERE clause is impossible.
CREATE OR REPLACE VIEW public.shadow_executions_production AS
  SELECT * FROM public.shadow_executions WHERE cohort = 'production';

REVOKE ALL ON public.shadow_executions_production FROM anon, authenticated;
GRANT SELECT ON public.shadow_executions_production TO service_role;

ALTER TABLE public.shadow_engine_state
  ADD COLUMN IF NOT EXISTS candidate_capture_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS candidate_enrolment_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS candidate_rows_per_run smallint NOT NULL DEFAULT 30;

-- Sibling fan-out must never clone a research candidate.
...
BEGIN
  ...
  -- Stage 4 isolation: research candidates get no Replay-V2 sibling.
  IF coalesce(NEW.cohort, 'production') <> 'production' THEN
    RETURN NEW;
  END IF;
  ...
  BEGIN
    ...
    INSERT INTO public.shadow_executions (
      ...
      NEW.plan_id, 2, 'single_exit_first_target', NEW.cohort,
...
-- Payoff research reads the production view too; cohort scoping is now structural.
CREATE OR REPLACE FUNCTION public.recompute_payoff_stats(_model_version smallint DEFAULT 1, _replay_version smallint DEFAULT 1, _execution_policy text DEFAULT 'legacy_best_target_touched'::text, _horizon_hours integer DEFAULT 24)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
