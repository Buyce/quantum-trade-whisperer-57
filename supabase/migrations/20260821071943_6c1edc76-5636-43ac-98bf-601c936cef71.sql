-- 1. Baseline documents become service-role/admin only.
DROP POLICY IF EXISTS baseline_snapshots_readable_by_authenticated ON public.baseline_snapshots;
REVOKE SELECT ON public.baseline_snapshots FROM authenticated;

-- 2. Preserve Tier-0 volatility boundaries prospectively.
ALTER TABLE public.regime_snapshots ADD COLUMN IF NOT EXISTS vol_t1 numeric;
ALTER TABLE public.regime_snapshots ADD COLUMN IF NOT EXISTS vol_t2 numeric;
...
BEGIN
  ...
  -- Tier 0 is included so the volatility tercile boundaries the live scanner
  -- read during this iteration are preserved instead of being destroyed by the
  -- next rebuild. Historical snapshots keep no Tier-0 rows: none are fabricated.
